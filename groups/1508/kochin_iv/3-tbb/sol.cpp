...
double* Gauss(double* image, int width, int height, double Sigma = 3, int radius = 1)
{
	...
	tbb::parallel_for(tbb::blocked_range<int>(radius, height-radius), [&image, &radius, &width, &kernel,&result](const tbb::blocked_range<int>& h){
		for (int y = h.begin(); y < h.end(); y++)
			for (int x = radius; x < width - radius; x++)
			{
				for (int i = -radius; i <= radius; i++)
					for (int j = -radius; j <= radius; j++) {
						...
						//39042 height - 291 width - 354 len - 309042
						//for GREEN channel
