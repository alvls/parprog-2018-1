...
{
	int threads_count = tbb::task_scheduler_init::default_num_threads();
	int chunk_size = n / threads_count ? n / threads_count : 1;
	tbb::parallel_for(tbb::blocked_range<int>(0, threads_count), [&a, chunk_size](auto range)
	{
		for (auto i = range.begin(); i != range.end(); ++i)
		{
			auto first = i * chunk_size;
			auto last = first + chunk_size - 1;
			HoaraQuickSort_OMP(a, first, last);
		}
	}
	);
