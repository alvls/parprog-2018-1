...
void solve(int ** graph, Vertex * vertexes, int n) {
	...
	for (;;) {
		bool any = false;
		tbb::parallel_for(tbb::blocked_range<int>(0, m), [&any, &m, &d, &e, &INF, &p](const tbb::blocked_range<int>& h) {
	...
	tbb::parallel_for(tbb::blocked_range<int>(0, n), [&vertexes, &d, &p](const tbb::blocked_range<int>& h) {
