...
int main(int argc, char *argv[])
{
	...
	fread(&timeSer, sizeof(timeSer), 1, stdin);
	fread(&timePar, sizeof(timePar), 1, stdin);
	fread(&n, sizeof(n), 1, stdin);
	...
	if (diff < 0.0001)
	{
		if (resultFile.is_open())
		{
			...
			resultFile << "Parallel version time is " << timePar << "\n";
			resultFile << "Serial version time is " << timeSer;
		}
