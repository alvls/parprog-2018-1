...
public:
    ...
    static void prepareTask(PrepareData &data, vector<MatrixCCS> &tmp, const MatrixCCS &m, int N)
    {
        ...
        for (int i = 0; i < data.numTask; i++)
            data.task[i] = Task(i*sizeTask, min((i + 1)*sizeTask, m.N), i % data.numTask);
        int lastPointerM = 0;
        for (int i = 0; i < data.numTask; i++)
        {
    ...
    void quickParallelMult(MatrixCCS &m, PrepareData &data, vector<MatrixCCS> &tmp)
    {
        class FunctorTBB
        {
        private:
        ...
        int numTask = data.numTask;
      
        vector<int> *cols = &rows;
        MatrixCCS *p = 0;

        FunctorTBB f(this, &m, &data.task, &tmp, &data.elCountM);
        tbb::parallel_for(tbb::blocked_range<size_t>((size_t)0, data.task.size(), (size_t)4), f);
    }
